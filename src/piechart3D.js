...
function prepearData(_data, fontSize, height) {
  ...
  const pi1 = data
    .map((d) => (d.endAngle <= Math.PI / 2 ? d : null))
    .filter((d) => d !== null)
    .sort((a, b) => b.value - a.value);
  const pi2 = data
    .map((d) => (d.endAngle > Math.PI / 2 && d.endAngle <= Math.PI ? d : null))
    .filter((d) => d !== null)
    .sort((a, b) => a.value - b.value);
  const pi3 = data
    .map((d) => (d.endAngle > Math.PI && d.endAngle <= (3 * Math.PI) / 2 ? d : null))
    .filter((d) => d !== null)
    .sort((a, b) => a.value - b.value);
  const pi4 = data
    .map((d) => (d.endAngle > (3 * Math.PI) / 2 ? d : null))
    .filter((d) => d !== null)
    .sort((a, b) => b.value - a.value);
...
function labelPath(d, rx, ry, h) {
  ...
  path.push(
    'L',
    (rx + 14) * (midAngle(d) > (3 / 2) * Math.PI || midAngle(d) < Math.PI / 2 ? 1 : -1),
...
function prepareConfig(userConfig) {
  let conf = {
    ...
    size: 100,
    label: (d) =>
      `${d.data.label ? d.data.label : ''}(${Math.round(
...
piechart3D.update = (chartId, _data, userConfig) => {
  ...
  const { height } = select(`#${chartId}`).node().getBoundingClientRect();
  const { width } = select(`#${chartId}`).node().getBoundingClientRect();
  ...
  function drowingEnd(d) {
    select(`#${id}-slice-${d.index}`).on('click', (d2) =>
      conf.animatedSlices ? onClick(id, d2, rx, ry, conf.onSliceSelect) : null,
    );
  }
  ...
  if (slicesCount > data.length) {
    ...
    for (let i = data.length - slicesCount - 1; i >= 0; i -= 1) {
      ...
      select(`#${id}`).select('.lines').append('path').attr('class', 'label-path');

      select(`#${id}`).select('.labels').append('text').attr('class', 'label');

      select(`#${id}`).select('.tooltips').append('text').attr('class', 'tooltip');
    }
  }
  ...
  select(`#${id}`)
    ...
    .attrTween('transform', function (d) {
      ...
      return (t) => {
        ...
        return `translate(${
          (rx + 16) * (midAngle(i(t)) > (3 / 2) * Math.PI || midAngle(i(t)) < Math.PI / 2 ? 1 : -1)
        }, 
    ...
    labels
      ...
      .attrTween('transform', (d) => {
        ...
        return (t) => {
          ...
          const labelPathLength = 1 + conf.h / rx / 2;
          return `translate(${
            (rx + 16) * (midAngle(d2) > (3 / 2) * Math.PI || midAngle(d2) < Math.PI / 2 ? 1 : -1)
          }, 
