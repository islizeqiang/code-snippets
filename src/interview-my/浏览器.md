...
1. 重绘
   1. 颜色改变
   2. visibility 从 visible 变为 hidden 等
2. 回流重排
   1. 添加 删除 更新 DOM
   2. 移动 DOM 位置，改变布局
   3. 改变窗口大小等
   4. 读写 offset，scroll，client 等属性或调用 window.getComputedStyle，因为浏览器并不清楚上一个循环内 DOM 有没有改变位置，必须立即重新布局才能计算 DOM 位置
   5. display 转换成 none

offsetTop 和 Top 的区别在于，前者返回数字，并且仅只读，后者返回字符串，包含单位，可读写，但没指定过 top 的话，会返回字符串。

如何避免：

1. 避免频繁操作 style，用 class 修改的方式
2. 对 resize，scroll 进行防抖和节流
3. 可将要变化的层提升为合成层
...
1. 可以是布尔值 useCapture
   ...
   - 为 true 时，在捕获阶段调用，默认为false，在冒泡阶段调用。
