...
1. 重绘
   ...
   由于节点的几何属性发生改变或者由于样式发生改变而不会影响布局的，称为重绘，例如`visibility`, `color`、`background-color`等

2. 回流

   1. 一个 DOM 元素的几何属性变化，常见的几何属性有`width`、`height`、`padding`、`margin`、`left`、`top`、`border` 等等, 这个很好理解。
   2. 使 DOM 节点发生`增减`或者`移动`。
   3. 读写 `offset`族、`scroll`族和`client`族属性的时候，浏览器为了获取这些最新的值，需要进行回流操作。
   4. 调用 `window.getComputedStyle` 方法。

如何避免：

1. 避免频繁操作style，用class修改的方式
2. 对resize，scroll进行防抖和节流
3. 可将要变化的层提升为合成层   
